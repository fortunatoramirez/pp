...
# **Implementación de proyectiles disparados por el dragón**

En esta sección se agregará interacción adicional al personaje, permitiendo que el dragón dispare proyectiles mientras se desplaza por el escenario. El objetivo es aplicar conceptos de programación orientada a objetos dentro de un entorno gráfico, manejar múltiples elementos dinámicos en pantalla y actualizar su comportamiento en función de la dirección del personaje.

Los disparos se generarán cuando el usuario presione una tecla determinada, y avanzarán en la dirección correspondiente al último movimiento del dragón. Cada proyectil permanecerá activo hasta salir de la pantalla.

---

## **4.1. Estructura básica de un proyectil**

Un proyectil puede representarse como un diccionario o como un objeto. En esta práctica se empleará un diccionario para mantener el enfoque accesible:

Atributos necesarios:

* `x`: posición horizontal
* `y`: posición vertical
* `dx`: velocidad horizontal
* `dy`: velocidad vertical
* `imagen`: superficie que representa el proyectil

---

## **4.2. Creación de la lista de proyectiles**

Antes del bucle principal del juego:

```python
proyectiles = []
```

Se carga una imagen sencilla, por ejemplo un pequeño fuego o bola de energía (puede ser un PNG de 8–32 px). Temporalmente, para propósitos didácticos, se usará un rectángulo generado con código:

```python
proyectil_img = pygame.Surface((12, 6))
proyectil_img.fill((255, 200, 0))  # Color amarillo-fuego
```

---

## **4.3. Función para crear un proyectil según la dirección del dragón**

Esta función coloca el proyectil frente al dragón y asigna una velocidad ajustada a la orientación actual:

```python
def crear_proyectil(x, y, direccion):
    velocidad = 10

    if direccion == "right":
        return {"x": x + 40, "y": y + 20, "dx": velocidad, "dy": 0, "img": proyectil_img}
    elif direccion == "left":
        return {"x": x - 10, "y": y + 20, "dx": -velocidad, "dy": 0, "img": proyectil_img}
    elif direccion == "up":
        return {"x": x + 20, "y": y - 10, "dx": 0, "dy": -velocidad, "img": proyectil_img}
    elif direccion == "down":
        return {"x": x + 20, "y": y + 50, "dx": 0, "dy": velocidad, "img": proyectil_img}
```

Con esto, el disparo **siempre sale desde el frente de la orientación actual**.

---

## **4.4. Disparo mediante el teclado**

En el bucle principal, dentro de la lectura de teclado:

```python
teclas = pygame.key.get_pressed()

if teclas[pygame.K_SPACE]:   # Barra espaciadora para disparar
    proyectiles.append(crear_proyectil(dragon_x, dragon_y, direccion_actual))
```

---

## **4.5. Actualización del movimiento de los proyectiles**

En el cuerpo del bucle principal del juego:
...
## **4.6. Eliminación de proyectiles fuera de pantalla**

Es importante depurar constantemente la lista para evitar acumulación innecesaria de memoria:

```python
proyectiles = [
    p for p in proyectiles
    if 0 <= p["x"] <= ANCHO and 0 <= p["y"] <= ALTO
]
```

---

## **4.7. Dibujo de los proyectiles en pantalla**

Después de dibujar al dragón:

```python
for p in proyectiles:
    pantalla.blit(p["img"], (p["x"], p["y"]))
```

---

# **4.8. Código completo integrado de disparo**

Este fragmento va dentro del bucle principal (luego del movimiento del dragón y antes del `pygame.display.update()`):
...
# **4.9. Actividad de comprobación**

El estudiante deberá:

1. Explicar por qué es necesario eliminar proyectiles que salen de la pantalla.
2. Modificar el disparo para que el dragón pueda lanzar proyectiles diagonales cuando se presionan dos teclas de dirección.
3. Sustituir el proyectil temporal por una imagen PNG con transparencia.
4. Ajustar la velocidad del proyectil e incluir una animación de impacto cuando llegue al borde derecho de la pantalla.
